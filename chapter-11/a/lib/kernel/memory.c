...
static void* palloc(struct pool* m_pool) {
    ...
    bitmap_set(&m_pool->pool_bitmap, bit_idx, 1); // 将次位bit_idx置为1
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    /** 注意**************************************
     * 执行*pte, 会访问到空的pde. 所以确保pde创建完成后才能执行*pde,
     * 否则会引发page_fault. 因此*pde为0时, *pte只能出现在下面else
     * 语句块中的*pde后面.
     * ******************************************
    */
    /** 先在页目录内判断目录项的P位, 若为1, 则表示该表已存在. */
        // 页目录项和页表项的第0位为P, 此处判断目录uu项是否存在
    if (*pde & 0x00000001) {
        ASSERT(!(*pte & 0x00000001));
        // 只要是创建页表, pte就应该不存在, 多判断以下放心
        if (!(*pte & 0x00000001)) {
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        }
        else {
            // 目前应该不会执行到这儿, 因为上面的ASSERT会先执行
            PANIC("pte repeat");
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        }
    }
...
void* get_a_page(enum pool_flags pf, uint32_t vaddr) {
    ...
    // NOTE： 此处与原文不一致！！！
        ...
        PANIC("get_a_page: kernel is not allowed to alloc userpace of user is not allowed to alloc kernelspace by get_a_page");
    }
...
static void mem_pool_init(uint32_t all_mem) {
   ...
   /**
    * 内核内存池和用户内存池位图 ***************************************
    * 位图或静态的数组需要在编译的时候知道其长度,
    * 而我们需要根据总内存大小算出需要多少字节,
    * 所以改为指定一块内存来生成位图.
    * *****************************************************
    * 内核使用的的最高地址是0xc009f000, 这是主线程的栈地址.
    * 内核大小预计为70KB左右.
    * 32MB内存占用的位图为2KB
