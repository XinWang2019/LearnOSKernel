...
static void intr_keyboard_handler(void) {
    ...
    /* 若为通码，只处理数组中定义的键以及alt_right和ctrl键，全是make_code */
        ...
        bool shift = false; // 判断是否与shift键组合，用来在一维数组中索引对应的字符
        ...
        if ((scancode < 0x0e) || (scancode == 0x29) || \
            ...
            else if (shift_down_last || caps_lock_last) { // 如果shift和capslock任意键被按下
                shift = true;
            }
            else {
                shift = false;
        ...
        uint8_t index = (scancode &= 0x00ff); // 将扫描码的高字节置为0,主要针对高字节是e0的扫描码
