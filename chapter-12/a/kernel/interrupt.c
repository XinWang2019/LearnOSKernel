...
struct gate_desc {
    ...
    uint16_t selector;
    uint8_t  dcount;                                       // 此项位双字计数字段, 是门描述符中的第四个字节, 此项固定值, 不用考虑
    uint8_t  attribute;
...
static void pic_init(void) {
    ...
    outb(PIC_M_DATA, 0x20); // ICW2: 起始中断向量为0x20, 即IR[0-7]为0x20~0x27
    outb(PIC_M_DATA, 0x04); // ICW3: IR2借从片
...
/* 在中断处理程序数组第vector_no个元素中注册安装中断处理程序 function */
void register_handler(uint8_t vector_no, intr_handler function) {
    /* idt_table数组中的函数是在进入中断后根据中断向量号调用的，
     * 见kernel/kernel.S的call[idt_table + i*4] */
